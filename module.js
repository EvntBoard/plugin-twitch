...
class TwitchEvntBoard {
  ...
  async load() {
    try {
      this.bus.newEvent({ event: 'twitch-load' })
      ...
      this.chatClient.onHosted(async (channel, byChannel, auto, viewers) => {
        this.bus.newEvent({ event: 'twitch-hosted', channel, auto, viewers })
      })
  ...
  async unload() {
    ...
    this.currentId = null
    this.connected = false
    this.bus.newEvent({event: 'twitch-unload'});
  }
